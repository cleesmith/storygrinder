...
async function checkTextFile(filePath) {
  try {
    ...
    const allWords = fileContent
      ...
      .replace(/[^\w\s']/g, ' ')  // Keep apostrophes for contractions, remove other punctuation
...
if (!filePath) {
  console.error('Usage: node spellchecker.js <path-to-text-file>');
  console.error('Example: node spellchecker.js ~/writing/HattieGetsAGun/manuscript.txt');
